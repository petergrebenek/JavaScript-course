...
window.addEventListener("DOMContentLoaded", () => {
  ...
  Object(_modules_timer__WEBPACK_IMPORTED_MODULE_6__["default"])(".timer", "2022-09-19");
});
...
function forms(formSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    form.addEventListener("submit", e => {
      ...
      const formData = new FormData(form); // const object = {}; // Необходимо, чтобы преобразовать formData
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  const modalTrigger = document.querySelectorAll(triggerSelector),
        modal = document.querySelector(modalSelector); // modalCloseBtn = document.querySelector("[data-close]"); // если элемент создан динамечески обработчико событий на нем работать не будет, по этому закомментирована
...
function slider({
  ...
  const slides = document.querySelectorAll(slide),
        ...
        width = window.getComputedStyle(slidesWrapper).width; // 1 Вариант

  let slideIndex = 1;
  /**
  showSlides(slideIndex);
    if (slides.length < 10) {
    // Отображение общего кал-ва слайдов
  ...
  */
  ...
  prev.addEventListener("click", () => {
    ...
    dots.forEach(dot => dot.style.opacity = ".5");
    dots[slideIndex - 1].style.opacity = 1;
  }); // _______________________Навигация для слайдера__________________________
...
function timer(id, deadline) {
  ...
  function setClock(selector, endTime) {
    ...
    function updateClock() {
      const t = getTimeReamaining(endTime); // Вписываем данные на страницу
